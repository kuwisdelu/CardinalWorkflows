...
First, we plot the single-ion images for $m/z$ 215.3, known to be more abundant in normal tissue (right) \cite{Dill}, shown in \figref{ionimages215}.
...
Likewise, we plot the single ion images for $m/z$ 885.7, known to be more abundant in cancerous tissue (left) \cite{Dill}, shown in \figref{ionimages886}.
...
plot(summary(rcc.cv.pls), main="PLS-DA accuracy")
@
...
plot(summary(rcc.cv.opls), main="O-PLS-DA accuracy")
@
...
As with PLS-DA, we not plot the images for the O-PLS-DA fitted values, to visually show the predictions, shown in \figref{oplsimages}.
...
% save(rcc.pca, rcc.cv.pls, rcc.pls, rcc.cv.opls, rcc.opls, file="~/Documents/Developer/Projects/CardinalWorkflows/data/rcc_analyses.rda", compress="xz")
% @
