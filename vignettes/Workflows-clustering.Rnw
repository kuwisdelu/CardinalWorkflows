...
\begin{subfigure}{.3\textwidth}
  \centering
\includegraphics{pig206-optical.pdf}
\caption{\small H\&E}
\label{fig:optical}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
...
This section demonstrates the spatial shrunken centroids clustering (SSC) method for statistical analysis we introduce in \Rpackage{Cardinal} in the \verb|spatialShrunkenCentroids| method.
...
mycol <- c("#FD9827", "#42FD24", "#CDFD34", "#1995FC", "#3524FB", "#FC0D1B", "#FC23D9")
...
We now perform spatial segmentation on MS images for which the ground truth is known.
...
<<eval=TRUE>>=
data(bierbaum, bierbaum_analyses)
@
...
<<eval=FALSE>>=
bierbaum.norm <- normalize(bierbaum, method="tic")
@
...
<<eval=FALSE>>=
bierbaum.peaklist <- peakPick(bierbaum.norm, pixel=seq(1, ncol(bierbaum), by=10), method="simple", SNR=6)

bierbaum.peaklist <- peakAlign(bierbaum.peaklist, ref=bierbaum.norm, method="diff", units="ppm", diff.max=200)

bierbaum.peaklist <- peakFilter(bierbaum.peaklist, method="freq", freq.min=ncol(bierbaum.peaklist) / 100)

bierbaum.peaks <- reduceDimension(bierbaum.norm, ref=bierbaum.peaklist, type="height")
@
...
<<eval=FALSE>>=
set.seed(1)
bierbaum.sscg <- spatialShrunkenCentroids(bierbaum.peaks, r=c(1,2), k=c(10,15), s=c(0,3,6,9), method="gaussian")

set.seed(1)
bierbaum.ssca <- spatialShrunkenCentroids(bierbaum.peaks, r=c(1,2), k=c(10,15), s=c(0,3,6,9), method="adaptive")
@


As in \secref{numberofsegments}, we plot the predicted number of classes against the shrinkage parameter $s$ to help determine the most appropriate model parameters for the data. \figref{bierbaumchooseg} shows this plot for the segmentations with Gaussian weights, and \figref{bierbaumchoosea} shows this for adaptive weights.

<<bierbaumchooseg>>=
plot(summary(bierbaum.sscg))
@

<<bierbaumchoosea>>=
plot(summary(bierbaum.ssca))
@
...
We plot the segmentation for $r = 1, k = 10, s = 3$, resulting in 8 segments, shown in \figref{bierbaumsscg}.


<<bierbaumsscg, eval=FALSE>>=
mycol <- c("#5C605C", "#4D2C36", "#A1A1A1", "#999999", "#B02020", "#1B1B1B", "#901010", "#906565")

image(bierbaum.sscg, model=list(r=1, k=10, s=3), key=FALSE, col=mycol)
@

We also plot the segmentation for $r = 2, k = 10, s = 3$, resulting in 8 segments, shown in \figref{bierbaumssca}.


<<bierbaumssca, eval=FALSE>>=
mycol <- c("#1B1B1B", "#A1A1A1", "#906565", "#999999", "#B02020", "#901010", "#5C605C", "#4D2C36")

image(bierbaum.ssca, model=list(r=2, k=10, s=3), key=FALSE, col=mycol)
@
...
\begin{subfigure}{.3\textwidth}
  \centering
\includegraphics{bierbaum-optical.png}
\caption{\small Cardinal painting}
\label{fig:bierbaumoptical}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
  \centering
<<fig=TRUE, echo=FALSE>>=
...
% <<eval=FALSE, echo=FALSE>>=
% save(bierbaum, bierbaum.peaklist, bierbaum.peaks, file="~/Documents/Developer/Projects/CardinalWorkflows/data/bierbaum.rda", compress="xz")
% save(bierbaum.sscg, bierbaum.ssca, file="~/Documents/Developer/Projects/CardinalWorkflows/data/bierbaum_analyses.rda", compress="xz")
% @
