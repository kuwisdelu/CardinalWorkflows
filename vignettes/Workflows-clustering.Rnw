...
The top $m/z$ values for a segmentation can be queried using the \verb|topFeatures| method.

<<>>=
topFeatures(pig206.sscg, n=20)
@

This list can be filtered by the segment, model parameters, etc.

<<>>=
topFeatures(pig206.sscg, model=list(r=2, s=6, k=20), filter=list(classes=5))
@

See \verb|?topFeatures| for details. This will be explored further in \secref{sscinterpret}.
...
summaryPlots <- function(dataset, results, model, segment, name, col) {
  ...
  top <- topFeatures(results, n=3, model=model, filter=list(classes=segment))
